...
export default function Profile() {
  ...
  const fetchFollowingStatus = async () => {
    ...
    try {
      const response = await axios.get(
        import.meta.env.VITE_SERVER_URL + `/api/users/${currentUserId}`
      );
    ...
    const fetchFavorites = async () => {
      try {
        const { data: trackData } = await axios.get(
          import.meta.env.VITE_SERVER_URL + `/api/users/${userId}/favorites`
        );
        setTracks(trackData);
      } catch (error) {
    ...
    const fetchPosts = async () => {
      try {
        const { data: postData } = await axios.get(
          import.meta.env.VITE_SERVER_URL + `/api/users/${userId}/posts`
        );
        setPosts(postData);
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const fetchPlaylists = async () => {
      try {
        const response = await axios.get(
          import.meta.env.VITE_SERVER_URL + `/api/users/${userId}/playlists`
        );
  ...
  const getTabContent = () => {
    switch (activeTab) {
      case "favorites":
        return (
          <div className="mx-12 h-52 flex justify-center">
      ...
      case "playlists":
        return (
          <div className="mt-6 mx-24 h-52 flex justify-center">
            <div className="w-full max-w-4xl">
              {!!playlists.length &&
                playlists.map((playlist) => (
                  <PlaylistAsItem
                    ...
                    playlist={playlist}
                    manageMode={false}
                    showFavorite={currentUserId !== userId}
      ...
      case "posts":
        return (
          <div className="flex mt-6 mx-12 justify-center">
    ...
    try {
      ...
      if (isFollowing) {
        response = await axios.delete(
          import.meta.env.VITE_SERVER_URL + `/api/users/${userId}/follow`
        );
      } else {
          ...
          <div className="flex ml-20 mt-10">
            ...
            <div className="flex flex-col ml-20 mt-4 gap-4">
              ...
              <div className="flex gap-4 text-lg">
                <span>
                  {t("following") + ": " + user.following?.length || 0}
                </span>
                <span>
                  {t("followers") + ": " + user.followers?.length || 0}
                </span>
              </div>
              {currentUserId !== userId && (
                <div className="flex">
                  <button
                    ...
                    className="mx-4 py-1.5 bg-orange-300 rounded-3xl w-20"
                  >
                    {isFollowing ? t("followed") : t("follow")}
                  </button>
                  <button
                    onClick={handleMessaging}
                    className="py-1.5 bg-orange-300 rounded-3xl w-20"
                  >
                    {t("privateMessage")}
                  </button>
                </div>
            ...
            <div className="flex gap-20 justify-center text-lg mt-4 pb-3 mx-12 border-b">
              <button
                className={`px-3 py-1 rounded-2xl ${
                  activeTab === "favorites" ? "bg-orange-300" : ""
                }`}
              ...
              <button
                className={`px-3 py-1 rounded-2xl ${
                  activeTab === "playlists" ? "bg-orange-300" : ""
                }`}
